...
export function Navbar() {
  ...
  async function handleSearch(q) {
    ...
    const { data } = await supabase
      .from('posts').select('id,title,tag,profiles(username)')
  ...
  return (
    <nav className="main-nav" ref={searchRef}>
      <div className="nav-logo" onClick={() => navigate(user ? '/feed' : '/')}>
        X<span>p</span>ora
      </div>
      {user && (
            ...
            <div style={{ position: 'relative' }}>
              ...
              {showNotifs && (
                <div className="notif-panel">
                  ...
                  {notifs.length === 0 ? <div style={{ padding: '20px', textAlign: 'center', color: 'var(--text3)', fontSize: 14 }}>No notifications yet</div> :
                    notifs.map(n => (
